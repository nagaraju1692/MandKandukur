...
async function sendAnnouncementPush(announcement) {
  try {
    const { rows } = await query('SELECT token FROM expo_push_tokens')
    ...
    for (let index = 0; index < rows.length; index += 100) {
      const messages = rows.slice(index, index + 100).map(({ token }) => ({
        ...
        body: announcement.detail || announcement.description,
      ...
      if (!expoResponse.ok) throw new Error(`Expo push request failed with ${expoResponse.status}`)
    }
  } catch (error) {
    console.error('Unable to send announcement push notification:', error.message)
  }
}
...
app.post('/api/devices/push-token', async (request, response, next) => {
  try {
    await ensureSchema()
    const token = normalizeText(request.body.token)
    const deviceId = normalizeText(request.body.deviceId) || null
    const platform = normalizeText(request.body.platform) || null
    if (!isExpoPushToken(token)) return response.status(400).json({ error: 'A valid Expo push token is required' })

    const user = await getAuthenticatedUser(request)
    const { rows } = await query(
      `INSERT INTO expo_push_tokens (token, user_phone, device_id, platform)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (token) DO UPDATE SET
         user_phone = COALESCE(EXCLUDED.user_phone, expo_push_tokens.user_phone),
         ...
         updated_at = NOW()
       RETURNING token, user_phone AS "userPhone", device_id AS "deviceId", platform`,
      [token, user?.phone || null, deviceId, platform],
    )
...
app.post('/api/admin/announcements', async (request, response, next) => {
  try {
    ...
    await sendAnnouncementPush(rows[0])
    response.status(201).json({ data: rows[0] })
  } catch (error) {
    next(error)
  }
